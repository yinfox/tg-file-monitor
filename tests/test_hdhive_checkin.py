...
from app.app import (
    ...
    _extract_hdhive_checkin_action_ids_from_js,
...
class HDHiveCheckinTestCase(unittest.TestCase):
    ...
    def test_extract_checkin_action_ids_supports_whitespace_single_quote_and_checkin_alias(self):
        js_text = "let j=(0,k.createServerReference) ( '40efbc107064215e9eff178b0466274739ba7d9cb4',k.callServer,void 0,k.findSourceMapURL,'checkin');"
        self.assertEqual(
            _extract_hdhive_checkin_action_ids_from_js(js_text),
            ["40efbc107064215e9eff178b0466274739ba7d9cb4"],
        )
    ...
    def test_server_action_tries_multiple_page_paths_before_force_refresh(self):
        call_paths = []

        def fake_next_action_call(**kwargs):
            call_paths.append(kwargs.get("page_path"))
            if kwargs.get("page_path") == "/":
                return None, 404, "404 Not Found"
            return {"response": {"message": "签到成功", "points": 66}}, 200, '1:{"response":{"message":"签到成功","points":66}}'

        with patch("app.app._refresh_hdhive_checkin_action_id_if_needed", return_value="action"), \
             patch("app.app._get_hdhive_home_router_state_tree_json", return_value='["root"]'), \
             patch("app.app._hdhive_next_action_call_sync", side_effect=fake_next_action_call):
            ok, msg, points = _hdhive_do_checkin_via_server_action("https://hdhive.com", "foo=bar", "normal")

        self.assertTrue(ok)
        self.assertEqual(msg, "签到成功")
        self.assertEqual(points, 66)
        self.assertEqual(call_paths, ["/", "/manager"])
