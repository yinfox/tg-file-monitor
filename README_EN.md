...
当前最新镜像：`y1nf0x/tg-file-monitor:0.5.84`
...
### v0.5.84 (2026-04-28) - 当前版本
- ✅ 诗词红包新增积分下限与个数下限配置，只有达到门槛才会自动答题点击
- ✅ 门槛校验支持从红包文案解析积分与个数，解析不到必要字段时自动跳过
- ✅ 点击通知补充红包积分/个数信息，并新增门槛解析与点击回归测试

### v0.5.83 (2026-04-28)
- ✅ 诗词红包设置收口：移除普通红包关键词、按钮文本、极速点击与口令/验证码红包相关配置
- ✅ 新增诗词红包抢红包风控：支持最小点击间隔、每小时上限与随机延时抖动
- ✅ 诗词红包点击成功后统一发送通知，并补充风控与通知回归测试

### v0.5.82 (2026-04-28)
- ✅ 红包自动点击新增诗词填空识别，支持通过本地诗词题库匹配正确选项后点击
- ✅ 受限频道自动点击新增点击延时配置，可按频道设置 0-60 秒延迟，降低过快点击风险
- ✅ 新增诗词题库构建脚本与来源说明，并补充 OpenCC 依赖用于繁简转换
- ✅ 补充红包自动点击、诗词题库匹配与延时点击回归测试

### v0.5.80 (2026-04-28)
- ✅ 自助观影 Open API 增加本地限速、缓存与“仅走 Open API”选项，降低站点回退与重复请求压力
- ✅ 自助观影公共页新增访问授权与最近申请记录体验优化，申请结果与历史回看更稳定
- ✅ Emby 自动补缺与季匹配回退继续加固，补充覆盖回归测试
- ✅ 红包/验证码自动点击新增口令验证码识别与回复配置，补充 OCR 依赖
- ✅ Web 配置页与移动端界面细节优化，并同步示例配置与文档镜像版本
...
### v0.5.43 (2026-04-09)
- ✅ 修复 HDHive 自动/手动签到 action 刷新逻辑：改为从首页动态 chunk 列表中提取当前 live `checkIn` Server Action
- ✅ 当前站点下手动签到已恢复，真实链路验证返回“你已经签到过了，明天再来吧”并同步当前积分
- ✅ 补充覆盖首页动态 chunk 路径提取与签到回归的测试，避免再次回退到过期 action id
...
### v0.5.12 (2026-03-17)
- ✅ 自助观影季匹配增强：支持从嵌套字段/文件名中识别 S01/S02
...
- ✅ 配置/TV 白名单读取更稳：写入原子化，读取失败保留上次有效配置
- ✅ 启动回溯 TV 白名单（可配置回溯条数，默认 20，仅对启用 TV 过滤频道生效）

### v0.5.08 (2026-03-16)
- ✅ 自助观影季匹配兼容：支持 `S01+S02` / `S01-02` / `S01E02` 等标题格式
